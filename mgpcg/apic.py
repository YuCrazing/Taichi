...
particle_cx = ti.Vector(2, dt=ti.f32, shape=n_particle)
particle_cy = ti.Vector(2, dt=ti.f32, shape=n_particle)
...
@ti.func
def scatter(grid_v, grid_m, xp, cp, vp, stagger):
...
def particle_to_grid():
	...
	for k in particle_position:
		...
		scatter(velocities_u, weights_u, pos, particle_cx[k], vel.x, stagger_u)
		scatter(velocities_v, weights_v, pos, particle_cy[k], vel.y, stagger_v)
...
def gather(grid_v, xp, stagger):
	...
	for i in ti.static(range(3)):
		for j in ti.static(range(3)):
			offset = ti.Vector([i, j])
			weight = w[i][0] * w[j][1]
			grad_weight = ti.Vector([grad_w[i][0]*w[j][1], w[i][0]*grad_w[j][1]])
			vel += grid_v[base + offset] * weight
			cp  += grid_v[base + offset] * grad_weight
...
def grid_to_particle():
	...
	for k in ti.grouped(particle_position):
		...
		vel_x, c_x = gather(velocities_u, p, stagger_u)
		vel_y, c_y = gather(velocities_v, p, stagger_v)

		particle_cx[k] = c_x
		particle_cy[k] = c_y
		particle_velocity[k] = ti.Vector([vel_x, vel_y])
...
for frame in range(450):
	...
	gui.text('APIC', pos=(0.05, 0.95), color=0x0)
