...
collision_mask = ti.Vector(2, dt=ti.f32, shape=(n, n))
...
def init_particles():
	for i in ti.grouped(particle_position):
		...
		if i.x < int(n_particle/2):
			particle_position[i] = ti.Vector([ti.random()*0.5, ti.random()]) * 0.1 + ti.Vector([0.45, 0.75])
		else:
			particle_position[i] = ti.Vector([ti.random()*0.5, ti.random()]) * 0.1 + ti.Vector([0.5, 0.75])
...
def advect_particles(particle_position:ti.template(), particle_velocity:ti.template(), velocities:ti.template()):
	for i in ti.grouped(particle_velocity):
		...
		particle_velocity[i] -= ti.dot(particle_velocity[i], collision_mask[index])
...
def init_collision_mask():
	sphere_center = ti.Vector([0.5, 0.25])
	sphere_radius = 0.1
	for I in ti.grouped(collision_mask):
		collision_mask[I] = ti.Vector([0.0, 0.0])
		pos = I * 1.0 / n
		if (sphere_center-pos).norm() < sphere_radius:
			collision_mask[I] = (pos-sphere_center).normalized(1e-5)
...
def solve_divergence(velocities:ti.template(), divergences:ti.template()):
	...
	for I in ti.grouped(velocities):
		if collision_mask[I].norm_sqr() > 0:
			velocities[I] = ti.Vector([0.0, 0.0])
...
n_guide = 5
pre_mouse_pos = [None]*n_guide
cur_mouse_pos = [None]*n_guide
...
for frame in range(600 if record else 450000):
	...
	k = 32
	if record and frame < k:
		for i in range(len(pre_mouse_pos)):
			pre_mouse_pos[i] = cur_mouse_pos[i]
		
		x = [0.5-1.0*frame/k*0.5, 0.5-1.0*frame/k*0.25, 0.5, 0.5+1.0*frame/k*0.25, 0.5+1.0*frame/k*0.5]
		y = [4.0*x[0]*x[0], 16.0*(x[1]-0.25)*(x[1]-0.25), 1-1.0*frame/k, 16.0*(x[3]-0.75)*(x[3]-0.75), 4.0*(x[4]-1)*(x[4]-1)]


		# cur_mouse_pos = np.array([0.5, 0.5], dtype=np.float32) + frame * np.array([0.7, -1.0], dtype=np.float32) * 0.0001
		for i in range(len(pre_mouse_pos)):
			cur_mouse_pos[i] = np.array([x[i], y[i]], dtype=np.float32)
			# print(x, y)
			if pre_mouse_pos[i] is None:
				pre_mouse_pos[i] = cur_mouse_pos[i]
			apply_force(velocities, colors, pre_mouse_pos[i], cur_mouse_pos[i])
	else:
		pass
